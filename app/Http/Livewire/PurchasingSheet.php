    ...
    public function addorder(Clientorder $order)
    {
        if (count($this->ordenes) == 0) {
            ...
            foreach ($order->orderdetails as $detailorder) {
                $countrevision = $detailorder->installation ? $detailorder->installation->revisions->count() - 1 : 0;
                $revision = $detailorder->installation ?
                    $detailorder->installation->revisions()->orderBy('number_version', 'desc')->first() : null;  
                $revisionDetail = $revision ? $revision->revisiondetails->where('number_version', $countrevision) : [];
                foreach ($revisionDetail as $revisiondetail) {
                    $material = $revisiondetail->material;
            ...
            if ($this->ordenrep == false) {
                ...
                foreach ($order->orderdetails as $detailorder) { 
                    $countrevision = $detailorder->installation ? $detailorder->installation->revisions->count() - 1 : 0;
                    $revision = $detailorder->installation ?
                        $detailorder->installation->revisions()->orderBy('number_version', 'desc')->first() :
                        [];
                    $revisionDetail = $revision ? $revision->revisiondetails->where('number_version', $countrevision) : [];
                    foreach ($revisionDetail as $revisiondetail) {
                        $material = $revisiondetail->material;
