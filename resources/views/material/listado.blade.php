                ...
                <div class="card-tools">
                  ...
                  @endif
                    <div class="input-group input-group-sm" style="width: 135px;">
                        <input wire:model="search" type="text" class="form-control float-right" placeholder="Buscar Materiales...">
                    </div>
                </div>
            </div>
              <!-- /.card-header -->
            ...
            <tbody>
              ...
              @forelse($materials as $material)
                ...
                <tr class="registros">
                      ...
                      <td >
                        <button type="button" wire:click="explorar({{ $material->id }})" class="btn btn-primary btn-xs"><i class="fas fa-file-alt"></i>    Ver</button>
                        @if (auth()->user()->can('update', auth()->user()))
                            <button wire:click="update({{ $material->id }})" type="button" class="btn btn-success btn-xs">Actualizar</button>
                          @endif
                        @if (auth()->user()->can('delete', auth()->user()))
                            <button wire:click="destruir({{ $material->id }})" type="button" class="btn btn-danger btn-xs">Borrar</button>
                          @endif
                      </td>      
                </tr>
              @empty
